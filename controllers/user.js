...
module.exports = {
  ...
  auth2: async (req, res) => {
    ...
    let password = req.body.password;
    //let hashed_pass = crpto.md5(pass)
    const result = await sqlConn.promise().query(`SELECT * from accounts where username = '${username}'`)
    ...
    if (result[0][0]){
      ...
      const result2 = await sqlConn.promise().query(`insert into accounts (username, password) values ('${username}' ,'${password}');`)
